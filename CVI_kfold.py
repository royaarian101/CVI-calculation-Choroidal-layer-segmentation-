...
# train data
training_data = np.zeros(((num_image-n_test), im_size, im_size,1), dtype=np.float64)
...
for n in range(num_image):
    ...
    if (num in testarray):
        ...
        training_data[train,:,:] = x_img/255.0
        for i in range (number_class):
...
nfold = ...  #please enter number of folds

kf_nfold = KFold(n_splits=nfold, shuffle=True) 

pred = np.zeros_like((labels_test))
...
for train_index, val_index in kf_nfold.split(labels_train):
    ...
    weights=weights(...) # please choose one of the weights
    ...
    model.compile(optimizer=Adam(),  loss=[...], metrics=[dice_coef,specificity,f1_score,precision,recall,'accuracy'])#please choose one of the loss functions and fill in the blanck
    ...
    kfold_loss = kfold_loss + np.min(results.history["val_loss"])
    kfold_acc = kfold_acc + np.max(results.history["val_acc"])
    kfold_specificity = kfold_specificity + np.max(results.history["val_specificity"])
    kfold_sensitivity = kfold_sensitivity + np.max(results.history["val_recall"])
    kfold_dice = kfold_dice + np.max(results.history["val_dice_coef"])
    kfold_f1_score = kfold_f1_score + np.max(results.history["val_f1_score"])
...
preds_test_t = preds_test_t.copy()

kfold_loss = kfold_loss/nfold
kfold_acc = kfold_acc/nfold
kfold_specificity = kfold_specificity/nfold
kfold_sensitivity = kfold_sensitivity/nfold
kfold_dice = kfold_dice/nfold
kfold_f1_score = kfold_f1_score/nfold
########################################################
